...
int LabelSTSets::Main(int argc, char *argv[]) {
  ...
  // Initialize an Extractor according to the parse tree type.
  boost::scoped_ptr<Extractor> extractor;
  if (options.tree_type == kBitPar) {
    extractor.reset(new ExtractorBitPar(options));
  } else if (options.tree_type == kParZu) {
    extractor.reset(new ExtractorParZu(options));
  } else {
    assert(false);
  }
...
void LabelSTSets::ProcessOptions(int argc, char *argv[],
  ...
  usage_top << "Usage: " << name() << " [OPTION]... [FILE] \n\n"
            << "Read German parse trees from input, identify relations, and write labelled trees\nto output.  With no FILE argument, or when FILE is -, read standard input.\n\n"
            << "Options";
