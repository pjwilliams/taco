...
// e.g. using # as string delimiter #label="S" pcfg="-1.452" foo="blah\"blah"#
void ParseXmlTagAttributes(
    const std::string &s,
    std::vector<std::pair<std::string, std::string> > &attributes)
{
  ...
  while (true) {
    ...
    std::pair<std::string, std::string> &attr_pair = attributes.back();
    std::string &name = attr_pair.first;
    std::string &value = attr_pair.second;
    name = Trim(s.substr(begin, pos-begin));
    begin = s.find('"', pos+1);
    ...
    // TODO unescape \"
    value = s.substr(begin+1, pos-begin-1);
    begin = pos+1;
  }
}
...
bool ProcessAndStripXMLTags(string &line, SyntaxTree &tree, set< string > &labelCollection, map< string, int > &topLabelCollection )
{
  ...
  for (size_t xmlTokenPos = 0 ; xmlTokenPos < xmlTokens.size() ; xmlTokenPos++) {
    ...
    if(!isXmlTag(xmlTokens[xmlTokenPos])) {
      ...
      //std::string newText = unescape(xmlTokens[xmlTokenPos]);
      std::string newText = xmlTokens[xmlTokenPos];
      cleanLine += newText; // add to output
      wordPos += Tokenize(newText).size(); // count all the words
    }
    ...
    // process xml tag
      ...
      if (isClosed || isUnary) {
        ...
        ParseXmlTagAttributes(tagContent, node->m_attributes);
      }
    }
  }
